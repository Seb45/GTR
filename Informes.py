...
informe = st.sidebar.selectbox(
    ...
    # ("Turnos publicados", 'Cantidad por intervalo', 'GTR', 'GTR2', 'GTR3')
    ( 'GTR', 'GTR2', 'GTR3', 'GTR4', 'GTR5')
)
...
if informe=="Cantidad por intervalo":
    ...
    tipo_muid=st.sidebar.selectbox('mu', tuple(data_resumen_muid.mu_descripcion.unique())) 
    data_intervalos=pd.read_sql('SELECT * from s_cantidad_intervalo_exception where mu_descripcion="'+str(tipo_muid)+'" and TZ="'+tipo_geografia+'" and fecha="'+str(fecha_informe)+'"', con)
    ...
    if tipo_geografia=="America/Lima" or tipo_geografia=="America/Bogota":
        hora_local=ahora.hour-2
    else:
        hora_local=ahora.hour-0
    ...
    with st.expander("Ver Tabla Detalle por RAC"):
        ...
        data_detalle_rac=pd.read_sql('select agentName, start, stop, substr(logonID,1,8) as LogonId from agent_schedule_detail a inner join mu_description mu on mu.muid=a.muid inner join traduccion_exception e on a.exception=e.exception where TZ="'+tipo_geografia+'" AND substr(schedDate,4, 4)||"-0"||substr(schedDate,1, 1)||"-"||substr(schedDate,2, 2)="'+str(fecha_informe)+'" AND mu_descripcion="'+str(tipo_muid)+'" AND segmento="'+tipo_segmento+'" AND start<="'+str(t_intervalo)+'" and stop>"'+str(t_intervalo)+'" order by 1', con)
...
if informe=="Turnos publicados":
    ...
    data_turnos=pd.read_sql('SELECT * from s_turnos_diarios', con)
    
    fig2 = go.Figure()
    
    st.subheader ("Turnos publicados - Total Region")
 
    fig2.add_trace(go.Bar(x=data_turnos["fecha"], y=data_turnos["cantidad"],name="Cantidad racs"))

    fig2.update_xaxes(type='category')
    fig2.update_layout(height=450, width=1200)
...
if informe=="GTR":
    data_GTR_TEF=pd.read_sql('SELECT * from s_status_gtr_programa_estado', con)
    st.subheader ("Datos GTR Resumen")
    data_act_GTR_TEF=pd.read_sql('SELECT * from act_txt_tef', con)
    st.text(data_act_GTR_TEF.iloc[0,0])
    data_GTR_TEF_pivot=data_GTR_TEF.pivot(index='programa', columns='estado_encap', values='racs')
    data_GTR_TEF_pivot=data_GTR_TEF_pivot.fillna(0)
    # st.bar_chart(data_intervalo_pivot)
    # data_GTR_TEF_pivot_=data_GTR_TEF_pivot.reset_index()
    data_GTR_TEF_pivot["Total"]=data_GTR_TEF_pivot["Atencion"]+data_GTR_TEF_pivot["AUX"]
    st.dataframe(data=data_GTR_TEF_pivot, height=1200)

if informe=="GTR2":
    data_GTR_TEF=pd.read_sql('SELECT * from s_status_gtr_programa_estado_AVAIL', con)
    st.subheader ("Datos GTR Resumen")
    data_act_GTR_TEF=pd.read_sql('SELECT * from act_txt_tef', con)
    st.text(data_act_GTR_TEF.iloc[0,0])
    data_GTR_TEF_pivot=data_GTR_TEF.pivot(index='programa', columns='estado_encap', values='racs')
    ...
    # st.bar_chart(data_intervalo_pivot)
    # data_GTR_TEF_pivot_=data_GTR_TEF_pivot.reset_index()
    data_GTR_TEF_pivot["Total"]=data_GTR_TEF_pivot["Atencion"]+data_GTR_TEF_pivot["AVAIL"]+data_GTR_TEF_pivot["AUX"]
    data_GTR_TEF_pivot["Ocup"]=round(data_GTR_TEF_pivot["Atencion"]/(data_GTR_TEF_pivot["AVAIL"]+data_GTR_TEF_pivot["Atencion"])*100,2)
    data_GTR_TEF_pivot["Util"]=round((data_GTR_TEF_pivot["Atencion"]+data_GTR_TEF_pivot["AVAIL"])/(data_GTR_TEF_pivot["Total"])*100,2)
    
    st.dataframe(data=data_GTR_TEF_pivot, height=1200)


if informe=="GTR3":
    data_GTR_TEF=pd.read_sql('SELECT programa, nombre_personal, dni_personal, loginid, extn, "Motivo AUX", estado, "Split/Skill", tiempo, "Nombre de VDN" from s_ahora_programas', con)
    st.subheader ("Datos GTR")
    data_act_GTR_TEF=pd.read_sql('SELECT * from act_txt_tef', con)
    st.text(data_act_GTR_TEF.iloc[0,0])

    programa_select=st.multiselect('Programa', tuple(data_GTR_TEF.programa.unique()))

    data_GTR_TEF_prog=data_GTR_TEF[data_GTR_TEF["programa"].isin(programa_select)]
    st.dataframe(data_GTR_TEF_prog, width=1500)
    
if informe=="GTR4":
    data_GTR_TEF_detalle=pd.read_sql('SELECT * from s_gtr_progra_detalle_racs', con)
    st.subheader ("Datos GTR detalle RACs")
    data_act_GTR_TEF=pd.read_sql('SELECT * from act_txt_tef', con)
    st.text(data_act_GTR_TEF.iloc[0,0])

    programa_select=st.multiselect('Programa', tuple(data_GTR_TEF_detalle.programa.unique()))

    data_GTR_TEF_prog_detalle=data_GTR_TEF_detalle[data_GTR_TEF_detalle["programa"].isin(programa_select)]
    st.dataframe(data_GTR_TEF_prog_detalle, width=1500)
...
if informe=="GTR5":
    ...
    # programa_select=st.multiselect('Programa', tuple(data_GTR_TEF_detalle.programa.unique()))
    motivo_aux_select=st.multiselect('Motivo Aux', tuple(data_GTR_TEF_detalle.motivo_aux.unique()))
    tiempo_input=st.number_input('umbral', min_value=0, max_value=20000)
    # data_GTR_TEF_prog_detalle=data_GTR_TEF_detalle[data_GTR_TEF_detalle["programa"].isin(programa_select)]
    data_GTR_TEF_prog_detalle=data_GTR_TEF_detalle[data_GTR_TEF_detalle.motivo_aux.isin(motivo_aux_select)]
    data_GTR_TEF_prog_detalle=data_GTR_TEF_detalle[data_GTR_TEF_prog_detalle['Tiempo']*1>=300]
    st.dataframe(data_GTR_TEF_prog_detalle, width=1500)
